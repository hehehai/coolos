...
// box [track [transform [float]], inter-layer]

export interface BoxFloatProps
...
const BoxFloat = forwardRef<HTMLDivElement, BoxFloatProps>(
  (
    ...
    const [offset, handleDragStart, { getBaseData, offsetIsInit }] =
      useColorDrag({
        initOffset,
        inside,
        trackRef,
    ...
    const pointsLayer = useMemo(() => {
      ...
      return layerPoints.map((color, idx: number) => {
        ...
        if (baseData.transformWidth === 0 && baseData.transformHeight === 0)
          return
        ...
        return (
          <div
            key={idx}
            className={cn(
              "absolute h-[6px] w-[6px] rounded-full bg-white shadow-[0_0_0_1px_rgba(0,0,0,0.075)]",
              layerPointClassName
            )}
            style={{
              zIndex: idx + 1,
    ...
    const middleLayer = useMemo(() => {
      return (
        <div
          className={cn("absolute inset-0 z-[1]", layerGradientClassName)}
    ...
    return (
      <div ref={ref} className={cn("relative", className)} {...rest}>
        ...
        >
          <div
            ref={transformRef}
            className={cn("absolute", "z-1", { invisible: !offsetIsInit })}
            style={{
              left: offset.x,
              top: offset.y,
            }}
