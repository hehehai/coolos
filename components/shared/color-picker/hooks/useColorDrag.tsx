...
function useColorDrag({
  ...
  const [offset, setOffset] = useState<TransformOffset>(
    initOffset ?? { x: 0, y: 0 }
  )
  const [offsetIsInit, setOffsetIsInit] = useState(false)
  const dragMoveRef = useRef<((event: MouseEvent | TouchEvent) => void) | null>(
    null
  )
  const dragUpRef = useRef<((event: MouseEvent | TouchEvent) => void) | null>(
    null
  )
  ...
  const updateOffset: EventHandle = (e) => {
    ...
    const offsetX =
      Math.max(sideWidth, Math.min(pageX - trackX, trackWidth - sideWidth)) -
      centerOffsetX
    const offsetY =
      Math.max(sideHeight, Math.min(pageY - trackY, trackHeight - sideHeight)) -
      centerOffsetY
